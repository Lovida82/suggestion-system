        ...
        db.enablePersistence({ synchronizeTabs: true })
            .catch((err) => {
                if (err.code == 'failed-precondition') {
                    console.warn('여러 탭이 열려있어 캐싱을 활성화할 수 없습니다.');
                } else if (err.code == 'unimplemented') {
                    console.warn('브라우저가 캐싱을 지원하지 않습니다.');
                }
            });
        ...
        const effectScoreRanges = [
            ...
            { min: 50000001, max: Infinity, score: 55 } // 별도협의
        ];
        ...
        async function loadPendingVerifications() {
            try {
                ...
                console.error('검증대기 목록 로드 오류:', error);
        ...
        async function loadCompletedVerifications() {
            try {
                ...
                console.error('검증완료 목록 로드 오류:', error);
            }
        }
        ...
        async function loadAllSuggestions() {
            try {
                ...
                console.log('전체 제안 로드 완료:', snapshot.size, '건');
        ...
        async function completeVerification() {
            ...
            if (!confirm('검증을 완료하시겠습니까? 완료 후 제안서는 다음 단계로 진행됩니다.')) return;
            ...
            try {
                ...
                await db.collection('notifications').add({
                    ...
                    title: '유형효과 검증완료',
                    message: `${currentSuggestion.suggestionNumber} 제안서의 유형효과 검증이 완료되었습니다. 확정 금액: ${formatNumber(verifiedAmount)}원`,
                    type: 'verification',
                    read: false,
                    createdAt: firebase.firestore.FieldValue.serverTimestamp()
                });

                showToast('검증이 완료되었습니다. 다음 단계로 전달됩니다.', 'success');
                closeVerifyModal();
        ...
        async function rejectVerification() {
            ...
            if (!reason) {
                showToast('검증불가 사유를 반드시 입력해주세요.', 'error');
                return;
            }

            if (!confirm('검증불가 처리하시겠습니까? 제안서는 제안자에게 반려됩니다.')) return;
            ...
            try {
                ...
                await db.collection('notifications').add({
                    userId: currentSuggestion.userId,
                    title: '유형효과 검증불가',
                    message: `${currentSuggestion.suggestionNumber} 제안서가 검증불가 처리되었습니다. 사유: ${reason}`,
                    type: 'verification',
                    read: false,
                    createdAt: firebase.firestore.FieldValue.serverTimestamp()
                });

                showToast('검증불가 처리되었습니다.', 'success');
                closeVerifyModal();
                ...
                console.error('검증불가 처리 오류:', error);
        ...
        async function viewVerification(suggestionId) {
            ...
            if (isAdmin) {
                ...
                document.querySelector('.modal-footer').innerHTML = `
                    ...
                    <button class="btn btn-success" onclick="updateVerification('${suggestionId}')">수정 완료</button>
                `;
            } else {
                // 일반 검증자는 읽기만 가능
        ...
        async function updateVerification(suggestionId) {
            ...
            if (!confirm('검증정보를 수정하시겠습니까?')) return;
            ...
            try {
                ...
                showToast('검증정보가 수정되었습니다.', 'success');
                closeVerifyModal();
                ...
                console.error('검증정보 수정 오류:', error);
        ...
        // OpenAI API 키 설정
        const OPENAI_API_KEY = "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";

        // AI 분석 요청 (OpenAI API 직접 호출)
            ...
            try {
                ...
                const response = await fetch('https://api.openai.com/v1/chat/completions', {
                    ...
                    body: JSON.stringify({
                        ...
                        messages: [{
                            role: 'system',
                            content: '당신은 기업의 제안제도 시스템에서 유형효과를 검증하는 전문가입니다. 제안서의 내용을 분석하여 예상 절감액이 타당한지, 어떤 기준으로 검증해야 하는지 상세히 알려주세요.'
                        }, {
                            role: 'user',
                            content: `다음 제안서의 유형효과를 검증하려고 합니다. 검증 기준과 타당성 분석을 제공해주세요.

제안 제목: ${currentSuggestion.title}
제안자: ${currentSuggestion.proposer} (${currentSuggestion.department})

현황:
${currentSuggestion.currentSituation}

개선안:
${currentSuggestion.improvementPlan}

예상 절감액: ${formatNumber(currentSuggestion.expectedSaving)}원
산출 근거: ${currentSuggestion.savingBasis || '제공되지 않음'}
